...
- **Usage:** `Established (289★, 238 forks; PyPI 318k)` — a scored label (`Established` / `Emerging` / `Minimal`) followed by the evidence behind it: stars and forks, CRAN and PyPI totals, and any prose clause for programme adoption. Generated by `docs/fetch_usage.py`; never hand-written, and `docs/build.py` rejects a cell that does not start with a label.

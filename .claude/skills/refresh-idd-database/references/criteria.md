...
**The label is scored, not judged.** Since 2026-08-14 every `Usage` cell opens with `Established`, `Emerging` or `Minimal`, computed by `docs/fetch_usage.py` from a points total: 1 point per GitHub star and per fork, 1 per 1,000 all-time CRAN or PyPI downloads, 1 per 5 citations of a paper about the tool (1 per 20 where the cited paper is the science the tool implements rather than the software), 1 per country with documented use; >200 points `Established`, 30–200 `Emerging`, <30 `Minimal`. Do not assign a label by hand and do not restate the points in the file — run the script, which rewrites the column in place and records the breakdown in `docs/data/usage.json`.

`Usage` cell format: `Established (289★, 238 forks; PyPI 318k; 663 citations)`; `Emerging (CRAN 24k; 121 citations)`; `Emerging (South Africa's official national HIV estimates)`. The parenthetical is the evidence the label came from; prose clauses are preserved across re-runs, star/fork/download/citation figures are regenerated. The column was named `Adoption` and was renamed to `Usage` by user instruction.

**Check whose citations they are.** `fetch_usage.py` ends by listing rows whose `Publication` paper neither names the tool nor describes software — citations that accrue to the science or the data, not the code. `contactdata` is the clear case: ~1,000 citations of Prem et al.'s contact matrices, 90% of the package's points. Those citations count at a quarter rate (1 point per 20). The classification is a title heuristic (`cited_for` in `docs/data/usage.json`) and a method paper by the tool's own authors is often its canonical citation, so check the list at the end of each run and override with `"cited_for": "tool"` in `docs/data/usage_manual.json`, saying why in its `_why`.

...
5. **A deliberate family of tools built to a shared design philosophy to address related problems**, presented and maintained as a suite by its developers, even where the components do not declare code dependencies on each other (e.g. the Optima models, which share a modelling approach, a workflow, and a project identity rather than a single engine).

Test 5 is deliberately softer than tests 1–4. A shared software dependency is strong evidence of an ecosystem but is **not** a requirement: a group of tools designed to a common philosophy, solving adjacent problems, and presented together by their maintainers is an ecosystem in the sense that matters to a user choosing between them. Where an entry qualifies only under test 5, the `Interoperability basis` column says so explicitly, so the weaker claim is visible rather than hidden.
...
20 ecosystems, sorted alphabetically (case-insensitive) by name.

| Name | Lead institution(s) | Language(s) | Anchor tool | Interoperability basis | Components | Status | Description | Website |
|---|---|---|---|---|---|---|---|---|
| Atomica / Optima | Burnet Institute; Optima Consortium for Decision Science | Python | `atomica` (current engine); `optima` (legacy engine) | **Shared design philosophy (test 5)**, plus a partial common engine (test 1). The family shares one approach — a spreadsheet-defined compartmental model coupled to allocative-efficiency optimisation for budget allocation — and a common analysis workflow, with `atomica-tools` providing shared scaffolding for running Atomica-based country analyses. Optima HIV and Optima Nutrition still run on their own legacy engines rather than on Atomica, so the coupling is by design and workflow rather than by dependency throughout. | ~10 | Active | Compartmental disease and health-system models coupled to budget-optimisation methods, applied to national programme planning | [optimamodel.com](https://optimamodel.com) |
...
| epiworld | University of Utah, Division of Epidemiology | C++, R, Python | `epiworld` (header-only C++ library) | **Common core engine (test 1)** — `epiworldR`, `epiworldpy` and `epiworldRShiny` are all bindings or applications over the same header-only C++ library, so model semantics are identical across languages. Small and single-institution, but the coupling is unambiguous. | ~4 | Active | Fast agent-based epidemiological modelling with a shared C++ core exposed to R and Python | [uofuepibio.github.io/epiworldR](https://uofuepibio.github.io/epiworldR/) |
...
**Components.** `epiworld` (header-only C++ library), `epiworldR` (R bindings, on CRAN), `epiworldpy` (Python bindings), `epiworldRShiny` (Shiny interface).
...
**Conflicts of interest.** This file was compiled by Cliff Kerr, who works at the Institute for Disease Modeling and leads development of Starsim, which is one of the listed ecosystems. IDM is also associated with EMOD, another listed ecosystem, and with LASER, which is listed among the excluded candidates. The compiler is additionally a co-author on the Optima and Atomica publications, and Sciris — a shared dependency of the Starsim and Atomica ecosystems — is also an IDM project. Atomica / Optima and epiworld both qualify partly or wholly under test 5, the softest of the five tests, so it is worth stating plainly that one of the two is an ecosystem the compiler has a stake in and the other is not. Ordering within the table is alphabetical and implies no ranking. Corrections and additions are welcome.
